...
<template>
  <header class="w-full p-2 flex flex-row items-start justify-between">
    ...
    <div class="flex flex-row">
      ...
      <template v-else>
        <Keycap cap-style="large" class="ml-2" theme="mod" @click.native="$store.commit('SET_ACTIVE_MODAL', 'register')">Join</Keycap>
        <Keycap cap-style="large" class="ml-2" theme="accent" @click.native="$store.commit('SET_ACTIVE_MODAL', 'login')">Log In</Keycap>
      </template>
    </div>
  </header>
</template>
