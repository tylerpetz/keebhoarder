  ...
  >
    ...
    <footer class="absolute w-full bottom-0 right-0 left-0 p-4 pb-6 flex justify-between items-end z-10">
      ...
      <div class="flex items-end">
        <Keycap class="icon">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 438.5 438.5" class="fill-current w-6 h-6 py-1">
        ...
        <Keycap theme="mod">Website</Keycap>
      </div>
    </footer>
    <div class="absolute h-full w-full inset-0 flex items-center justify-center" :class="activeModal ? 'z-20' : 'z-0'">
      ...
      <SwapCaps v-show="activeModal === 'theme'" :selectedTheme="theme" @change="theme = $event" @close="activeModal = false" />
    </div>
  </main>
</template>
