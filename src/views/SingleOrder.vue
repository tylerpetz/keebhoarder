...
export default {
  ...
  mounted () {
    this.$store.dispatch('order/getOrderById', this.id)
  }
}
</script>
...
<template>
  <div
    v-if="$store.getters['order/currentOrder']"
    class="h-full p-3"
  >
      ...
      <div class="p-6 bg-theme-bg-d mb-8 flex flex-row justify-between items-center rounded shadow">
        <div>
          <h3 class="text-xl leading-6 font-medium text-theme-text flex items-center">
            ...
            <span class="ml-2 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800">
              {{ $store.getters['order/currentOrder'].public ? 'Public' : 'Private' }}
            </span>
          </h3>
        ...
        <div class="flex space-x-4">
          <Keycap
            cap-style="large"
            type="button"
            @click.native="$showModal('OrderModal', { props: { order: $store.getters['order/currentOrder'] }})"
          >
          ...
          <Keycap
            ...
            cap-style="large"
            type="button"
            @click.native="$showModal('ItemModal', { props: { orderId: $store.getters['order/currentOrder'].id }})"
          >
