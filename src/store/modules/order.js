...
export default {
  ...
  getters: {
    currentOrder: state => Object.keys(state.currentOrder).length ? state.currentOrder : null,
  ...
  actions: {
    getOrders ({ commit, getters }) {
      Vue.axios.get(`/orders?${getters.pagination}${getters.sorts}`).then(({ data }) => {
    ...
    getOrderById ({ commit }, orderId) {
      Vue.axios.get(`/orders/${orderId}`).then(({ data }) => {
        commit('SET_CURRENT_ORDER', data)
      })
    },
    createOrder ({ dispatch }, order) {
      Vue.axios.post('/orders', order).then(() => {
        dispatch('getOrders')
      })
    },
    updateOrder ({ commit, dispatch }, { order, updateCurrent = false }) {
      ...
      Vue.axios.patch(`/orders/${order.id}`, updatedOrder).then(({ data }) => {
        if (updateCurrent) {
          commit('SET_CURRENT_ORDER', data)
        }
        dispatch('getOrders')
    ...
    deleteOrder ({ dispatch }, orderId) {
      Vue.axios.delete(`/orders/${orderId}`).then(() => {
        dispatch('getOrders')
      })
    },
    // table options
